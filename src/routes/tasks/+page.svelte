<script>
	...
	async function editTask(t) {
		...
		if (!newTitle) return;
		const { error } = await supabase.from('tasks').update({ title: newTitle }).eq('id', t.id);
		if (error) return alert(error.message);
		await loadTasks();
	}
...
<div class="p-6">
	...
	<div class="flex gap-3 mb-4">
		<input class="input input-bordered" placeholder="Search title..." bind:value={search} on:input={loadTasks} />
				...
				<div class="card bg-base-100 shadow-xl p-4">
					<div class="flex justify-between items-start">
						...
						<div class="flex flex-col items-end gap-2">
							<span class="badge" class:badge-success={t.status === 'Completed'}>{t.status}</span>
							<span class="badge badge-outline">{t.priority}</span>

							<div class="flex gap-2 mt-3">
								<button class="btn btn-xs" on:click={() => toggleComplete(t)}>{t.status === 'Completed' ? 'Mark Pending' : 'Complete'}</button>
								<button class="btn btn-xs" on:click={() => editTask(t)}>Edit</button>
								<button class="btn btn-xs btn-error" on:click={() => removeTask(t.id)}>Delete</button>
							</div>
						</div>
					</div>
				</div>
